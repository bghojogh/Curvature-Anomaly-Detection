...
from sampleReduction_Shell import SR_Shell_Extraction
...
from sklearn.neural_network import MLPRegressor   # http://scikit-learn.org/stable/modules/generated/sklearn.neural_network.MLPRegressor.html
from sklearn.metrics import mean_squared_error  # http://scikit-learn.org/stable/modules/generated/sklearn.metrics.mean_squared_error.html#sklearn.metrics.mean_squared_error
from sklearn.metrics import mean_absolute_error  # http://scikit-learn.org/stable/modules/generated/sklearn.metrics.mean_absolute_error.html#sklearn.metrics.mean_absolute_error
from sklearn.preprocessing import StandardScaler


def main():
    # ----- Settings:
    method = 'CAD'     # 'SVD_python', 'SVD_Jordan', 'SPCA', 'FLDA', 'R1D', 'modified_R1D',
                                # 'NMF_sklearn', 'Dictionary_learning', 'ICA', 'QR_decomposition', 'LU_decomposition',
                                # 'sorted_by_distance_from_mean', 'simple_random_sampling', 'stratified_sampling', 'recursive_median',
                                # 'ENN', 'DROP1', 'DROP3', 'Shell_extraction', CAD, kernel_CAD, CAD_noRanking, kernel_CAD_noRanking, PSA
    ...
    demo = False
    if demo is False:
        dataset = 'image_segmentation'   # 'page_block', 'pima', 'spam', 'image_segmentation', 'yeast', 'wine', 'iris', 'fertility', 'balance', 'isolet', 'facebook', 'forest_fire', 'medical_cost'
        split_in_cross_validation_again = False
        shuffle_in_crossValidation = True
        rank_samples_again = False
        convert_from_old_wrong_in_DROP3 = False
        percentage_data = 20
        n_folds = 10
        ...
        regressors_for_experiments = ['LinearRegression', 'Ridge', 'Lasso', 'RandomForest', 'NeuralNetwork']
        # clusteringMethods_for_experiments = ['KMeans', 'Birch', 'SpectralClustering', 'DBSCAN']
        clusteringMethods_for_experiments = ['KMeans', 'Birch']
    ...
    if experiment == 'unsupervised 2D demo' or experiment == 'regression 2D demo' or experiment == 'classification 2D demo':
        ...
        # ----- ranking samples:
        if method != 'simple_random_sampling' and method != 'stratified_sampling' and method != "ENN" and method != "DROP1" and method != "DROP2" and method != "DROP3" and method != "Shell_extraction" and method != "CAD_noRanking" and method != "kernel_CAD_noRanking":
        ...
        if method == "ENN" or method == "DROP1" or method == "DROP2" or method == "DROP3" or method == "Shell_extraction" or method == "CAD_noRanking" or method == "kernel_CAD_noRanking":
            print('###### Numerosity reduction:')
            path_to_save = './saved_files/' + experiment + '/' + dataset + '/' + method + '/ranking/'
            if rank_samples_again:
                time_ranking = []
                percentage_kept = []
                kept_prototypes_indices = [None] * n_folds
                for fold_index in range(n_folds):
                    print('############################## Cross validation: fold number ' + str(fold_index + 1) + ' out of ' + str(n_folds) + ' folds')
                    # --- taking the X and y of train and test sets for classification:
                    X_train = X_train_in_folds[fold_index]
                    ...
                    # --- rank samples:
                    start_timer = time.time()
                    kept_prototypes_indices[fold_index] = numerosity_reduction_noRanking(X=X_train.T, Y=y_train.T, experiment=experiment, method=method)
                    end_timer = time.time()
                    time_ranking.extend([end_timer - start_timer])
                    percentage_kept.extend([100 * sum((kept_prototypes_indices[fold_index] == 1).ravel()) / X_train.shape[0]])
                time_ranking_average = np.asarray(time_ranking).mean()
                time_ranking_std = np.asarray(time_ranking).std()
                percentage_kept_average = np.asarray(percentage_kept).mean()
                percentage_kept_std = np.asarray(percentage_kept).std()
                print('############################## average time: ' + str(time_ranking_average) + ' +- ' + str(time_ranking_std))
                print('############################## average kept percentage: ' + str(percentage_kept_average) + ' +- ' + str(percentage_kept_std))
                save_variable(time_ranking, 'time_ranking', path_to_save=path_to_save)
                save_variable(time_ranking_average, 'time_ranking_average', path_to_save=path_to_save)
                save_variable(time_ranking_std, 'time_ranking_std', path_to_save=path_to_save)
                save_variable(percentage_kept, 'percentage_kept', path_to_save=path_to_save)
                save_variable(percentage_kept_average, 'percentage_kept_average', path_to_save=path_to_save)
                save_variable(percentage_kept_std, 'percentage_kept_std', path_to_save=path_to_save)
                save_variable(kept_prototypes_indices, 'kept_prototypes_indices', path_to_save=path_to_save)
                save_np_array_to_txt(time_ranking, 'time_ranking', path_to_save=path_to_save)
                save_np_array_to_txt(time_ranking_average, 'time_ranking_average', path_to_save=path_to_save)
                save_np_array_to_txt(time_ranking_std, 'time_ranking_std', path_to_save=path_to_save)
                save_np_array_to_txt(percentage_kept, 'percentage_kept', path_to_save=path_to_save)
                save_np_array_to_txt(percentage_kept_average, 'percentage_kept_average', path_to_save=path_to_save)
                save_np_array_to_txt(percentage_kept_std, 'percentage_kept_std', path_to_save=path_to_save)
            else:
                path_to_save = './saved_files/' + experiment + '/' + dataset + '/' + method + '/ranking/'
                file = open(path_to_save + 'time_ranking_average.pckl', 'rb')
                time_ranking_average = pickle.load(file); file.close()
                file = open(path_to_save + 'time_ranking_std.pckl', 'rb')
                time_ranking_std = pickle.load(file); file.close()
                file = open(path_to_save + 'percentage_kept_average.pckl', 'rb')
                percentage_kept_average = pickle.load(file); file.close()
                file = open(path_to_save + 'percentage_kept_std.pckl', 'rb')
                percentage_kept_std = pickle.load(file); file.close()
                print('############################## average time: ' + str(time_ranking_average) + ' +- ' + str(time_ranking_std))
                print('############################## average kept percentage: ' + str(percentage_kept_average) + ' +- ' + str(percentage_kept_std))
                file = open(path_to_save + 'kept_prototypes_indices.pckl', 'rb')
                kept_prototypes_indices = pickle.load(file); file.close()
                if convert_from_old_wrong_in_DROP3:
                    kept_prototypes_indices_wasSavedBefore = kept_prototypes_indices.copy()
                    percentage_kept = []
                    for fold_index in range(n_folds):
                        print('############################## Cross validation: fold number ' + str(fold_index + 1) + ' out of ' + str(n_folds) + ' folds')
                        # --- taking the X and y of train and test sets for classification:
                        ...
                        sr_Drop = SR_Drop(X=X_train.T, Y=y_train.T, n_neighbors=number_of_neighbors + 1)  # +1 because: in n_neighbors, the sample itself is counted
                        kept_prototypes_indices_1 = sr_Drop.ENN_prototype_selection()
                        kept_prototypes_indices_1 = kept_prototypes_indices_1.ravel().astype(int)
                        # --- DROP2 ranking after ENN (this is what I had saved before for DROP3 wrongly):
                        kept_prototypes_indices_2 = kept_prototypes_indices_wasSavedBefore[fold_index]  #--> take the saved file (which was saved before)
                        ...
                        n_samples_backup = (X_train.T).shape[1]
                        kept_prototypes_indices__ = np.zeros((1, n_samples_backup))
                        pivot = -1
                        for sample_index in range(n_samples_backup):
                            if kept_prototypes_indices_1[:, sample_index] == 1:
                                ...
                                if kept_prototypes_indices_2[:, pivot] == 1:
                                    kept_prototypes_indices__[:, sample_index] = 1
                        kept_prototypes_indices[fold_index] = kept_prototypes_indices__
                        percentage_kept.append( (np.count_nonzero(kept_prototypes_indices[fold_index]) / (kept_prototypes_indices[fold_index].shape[1])) * 100 )
                    percentage_kept_average = np.asarray(percentage_kept).mean()
                    percentage_kept_std = np.asarray(percentage_kept).std()
                    print('############################## After correction----> average kept percentage: ' + str(percentage_kept_average) + ' +- ' + str(percentage_kept_std))
            # ----- classification:
            # print('###### Classifications:')
            X_reduced, Y_reduced = [None] * n_folds, [None] * n_folds
            for fold_index in range(n_folds):
                # --- taking the X and y of train and test sets for classification:
                X_train = X_train_in_folds[fold_index]
                X_test = X_test_in_folds[fold_index]
                ...
                y_test = y_test_in_folds[fold_index]
                # --- reduce data:
                X_reduced[fold_index] = (X_train[kept_prototypes_indices[fold_index].ravel().astype(int)==1, :]).T
                Y_reduced[fold_index] = (y_train[kept_prototypes_indices[fold_index].ravel().astype(int)==1, :]).T
                # --- check if all classes have at least one training sample:
                if experiment == 'classification':
                    labels = np.unique(Y_reduced[fold_index].ravel())
                    labels_test = np.unique(y_test.ravel())
                    n_classes = len(labels_test)
                    if len(labels) != n_classes:
                        # it means we do not have any sample from at least one of the classes
                        # so, add one random sample from any class (for example the first sample of every class)
                        for label in labels_test:
                            index = np.argwhere(y_train.ravel() == label)[0]
                            X_reduced[fold_index] = np.hstack((X_reduced[fold_index], (X_train[index, :]).T))
                            Y_reduced[fold_index] = np.hstack((Y_reduced[fold_index], (y_train[index, :]).T))
            # recognition_rate_in_folds = do_classification(X_sorted=X_reduced, Y_sorted=Y_reduced, X_test_in_folds=X_test_in_folds, y_test_in_folds=y_test_in_folds, classifiers_for_experiments=classifiers_for_experiments, n_folds=n_folds)
            if experiment == 'classification':
                # ----- classification:
                print('###### Classifications:')
                recognition_rate_in_folds = do_classification(X_sorted=X_reduced, Y_sorted=Y_reduced, X_test_in_folds=X_test_in_folds, y_test_in_folds=y_test_in_folds, classifiers_for_experiments=classifiers_for_experiments, n_folds=n_folds)
                ...
                if experiment == 'regression':
                    print('###### Regressions:')
                    recognition_rate_in_folds = do_regression(X_sorted=X_reduced, Y_sorted=Y_reduced, X_test_in_folds=X_test_in_folds, y_test_in_folds=y_test_in_folds, regressors_for_experiments=regressors_for_experiments, n_folds=n_folds)
                elif experiment == 'clustering':
                    print('###### Clustering:')
                    recognition_rate_in_folds = do_clustering(X_sorted=X_reduced, Y_sorted=Y_reduced, X_test_in_folds=X_test_in_folds, y_test_in_folds=y_test_in_folds, clusteringMethods_for_experiments=clusteringMethods_for_experiments, n_folds=n_folds)
            # ----- saving results:
            path_to_save = './saved_files/' + experiment + '/' + dataset + '/' + method + '/accuracy/'
            save_variable(recognition_rate_in_folds, 'recognition_rate_in_folds', path_to_save=path_to_save)
            save_np_array_to_txt(recognition_rate_in_folds, 'recognition_rate_in_folds', path_to_save=path_to_save)
            # --- averaging the rates:
            recognition_rate_average = np.asarray(recognition_rate_in_folds).mean(axis=1)
...
def numerosity_reduction_noRanking(X, Y, experiment, method):
    if experiment == 'classification 2D demo' or experiment == 'classification':
        number_of_neighbors = 3
        if method == 'ENN':
            sr_edited_NN = SR_edited_NN(X=X, Y=Y, n_neighbors=number_of_neighbors + 1)  # +1 because: in n_neighbors, the sample itself is counted
            kept_prototypes_indices = sr_edited_NN.ENN_prototype_selection()
        elif method == 'DROP1':
            sr_Drop = SR_Drop(X=X, Y=Y, n_neighbors=number_of_neighbors + 1)  # +1 because: in n_neighbors, the sample itself is counted
            kept_prototypes_indices = sr_Drop.Drop1_prototype_selection()
        elif method == 'DROP2':
            sr_Drop = SR_Drop(X=X, Y=Y, n_neighbors=number_of_neighbors + 1)  # +1 because: in n_neighbors, the sample itself is counted
            kept_prototypes_indices = sr_Drop.Drop2_prototype_selection()
        elif method == 'DROP3':
            sr_Drop = SR_Drop(X=X, Y=Y, n_neighbors=number_of_neighbors + 1)  # +1 because: in n_neighbors, the sample itself is counted
            kept_prototypes_indices = sr_Drop.Drop3_prototype_selection()
    if experiment == 'classification 2D demo' or experiment == 'classification' or experiment == 'regression' or experiment == 'clustering':
        if method == 'Shell_extraction':
            percentage_data = 20
            sr_Shell_Extraction = SR_Shell_Extraction(X=X, Y=Y)
            kept_prototypes_indices = sr_Shell_Extraction.prototype_selection(mode=experiment, percentage_kept_data=percentage_data)
        if method == "CAD_noRanking":
            my_CAD = My_CAD(n_neighbors=10, n_components=None, anomalyDetection=False, kernel=None)
            # my_CAD.CAD_fit(X=X, display_scree_plot=False)
            # y_pred, scores = my_CAD.CAD_predict(X=X)
            # kept_prototypes_indices = my_CAD.CAD_prototype_selection(scores=scores, kernel_approach=False)
            kept_prototypes_indices = my_CAD.CAD_prototype_selection_for_classification(X=X, Y=Y, kernel_approach=False)
        if method == "kernel_CAD_noRanking":
            kernel = "rbf"  # rbf, polynomial
            my_CAD = My_CAD(n_neighbors=10, n_components=None, anomalyDetection=False, kernel=kernel)
            # my_CAD.kernel_CAD_fit(X=X, display_scree_plot=False)
            # y_pred, scores = my_CAD.CAD_predict(X=X)
            # kept_prototypes_indices = my_CAD.CAD_prototype_selection(scores=scores, kernel_approach=True)
            kept_prototypes_indices = my_CAD.CAD_prototype_selection_for_classification(X=X, Y=Y, kernel_approach=True)
...
def rank_samples(X, Y, experiment, method):
    ...
    if experiment == 'unsupervised 2D demo' or experiment == 'clustering':
        if method == 'SVD_python' or method == 'SVD_Jordan' or method == 'R1D' or method == 'modified_R1D' or method == 'NMF_sklearn' or method == 'Dictionary_learning' or method == 'ICA' or method == 'QR_decomposition' or method == 'LU_decomposition' or method == 'stratified_sampling':
            ...
            my_CAD.CAD_fit(X=X, display_scree_plot=False)
            y_pred, scores = my_CAD.CAD_predict(X=X)
            X_sorted, Y_sorted, scores_sorted = my_CAD.sort_samples(scores=scores, X=X, Y=Y, kernel_approach=False)
        elif method == "kernel_CAD":
            kernel = "rbf"  # rbf, polynomial
            my_CAD = My_CAD(n_neighbors=10, n_components=None, anomalyDetection=False, kernel=kernel)
            my_CAD.kernel_CAD_fit(X=X, display_scree_plot=False)
            y_pred, scores = my_CAD.CAD_predict(X=X)
            X_sorted, Y_sorted, scores_sorted = my_CAD.sort_samples(scores=scores, X=X, Y=Y, kernel_approach=True)
        elif method == "PSA":
            ...
            ranks = psa.rank_samples(X=X.T, y=Y.T, demo_mode=False, report_steps=False, HavingClasses=False, Apply_PCA=False, n_components=None, for_regression_task=False)
            X_sorted, Y_sorted, scores_sorted = psa.sort_samples_classWise(ranks=ranks, X=X.T, Y=Y, for_classification=False)
        ...
        if method == 'SVD_python' or method == 'SVD_Jordan' or method == 'R1D' or method == 'modified_R1D' or method == 'NMF_sklearn' or method == 'Dictionary_learning' or method == 'ICA' or method == 'QR_decomposition' or method == 'LU_decomposition':
            ...
            psa = PSA(save_ranks=False, add_noisy_samples_if_not_necessary=False, number_of_added_noisy_samples=0,
                        number_of_selected_samples_in_group=10, number_of_iterations_of_RANSAC=20)
            ranks = psa.rank_samples(X=X.T, y=Y.T, demo_mode=False, report_steps=False, HavingClasses=False, Apply_PCA=False, n_components=None, for_regression_task=True)
            X_sorted, Y_sorted, scores_sorted = psa.sort_samples_classWise(ranks=ranks, X=X.T, Y=Y, for_classification=False)
        ...
        if method == 'SPCA':
            ...
            # y_pred, scores = my_CAD.CAD_predict(X=X)
            # X_sorted, Y_sorted, scores_sorted = my_CAD.sort_samples(scores=scores, X=X, Y=Y, kernel_approach=False)
            X_sorted, Y_sorted, scores_sorted = my_CAD.sort_samples_for_classification(X=X, Y=Y, kernel_approach=False)
        elif method == "kernel_CAD":
            kernel = "rbf"  # rbf, polynomial
            my_CAD = My_CAD(n_neighbors=10, n_components=None, anomalyDetection=False, kernel=kernel)
            # my_CAD.kernel_CAD_fit(X=X, display_scree_plot=False)
            # y_pred, scores = my_CAD.CAD_predict(X=X)
            # X_sorted, Y_sorted, scores_sorted = my_CAD.sort_samples(scores=scores, X=X, Y=Y, kernel_approach=True)
            X_sorted, Y_sorted, scores_sorted = my_CAD.sort_samples_for_classification(X=X, Y=Y, kernel_approach=True)
        elif method == "PSA":
            psa = PSA(save_ranks=False, add_noisy_samples_if_not_necessary=False, number_of_added_noisy_samples=0,
                        number_of_selected_samples_in_group=30, number_of_iterations_of_RANSAC=20)  #--> number_of_selected_samples_in_group=10, number_of_iterations_of_RANSAC=20
            ranks = psa.rank_samples(X=X.T, y=Y.ravel(), demo_mode=False, report_steps=False, HavingClasses=True, Apply_PCA=False, n_components=None, for_regression_task=False)
            X_sorted, Y_sorted, scores_sorted = psa.sort_samples_classWise(ranks=ranks, X=X.T, Y=Y, for_classification=True)
    ...
    for classifier_index in range(len(classifiers_for_experiments)):
        ...
        for fold_index in range(n_folds):
            ...
            if classifier == 'SVM':
                ...
                # --------- train:
                clf = RF(max_depth=2, random_state=0, n_estimators=10)
                clf.fit(X=X_train, y=y_train)
            elif classifier == 'LR':
                # --------- train:
                clf = LR(solver="liblinear", multi_class="ovr")
                clf.fit(X=X_train, y=y_train)
            elif classifier == 'NB':
                # --------- train:
                clf = GaussianNB()
...
def do_regression(X_sorted, Y_sorted, X_test_in_folds, y_test_in_folds, regressors_for_experiments, n_folds):
    ...
    for regressor_index in range(len(regressors_for_experiments)):
        ...
        for fold_index in range(n_folds):
            ...
            y_test = y_test_in_folds[fold_index]
            zscore_normalization = StandardScaler()
            zscore_normalization.fit(X_train)
            X_train_normalized = zscore_normalization.transform(X_train)
            X_test_normalized = zscore_normalization.transform(X_test)
            if regressor == 'LinearRegression':
                # --------- train:
                clf = LinearRegression()
                clf.fit(X=X_train_normalized, y=y_train)
                ...
                if y_train.shape[0] == 1 or y_train.shape[1] == 1:
                    y_train = y_train.ravel()
                clf.fit(X=X_train_normalized, y=y_train)
            elif regressor == 'NeuralNetwork':
                # --------- train:
                clf = MLPRegressor(hidden_layer_sizes=(100,50,), activation='relu', solver='adam', max_iter=10000, learning_rate='adaptive')
                if y_train.shape[0] == 1 or y_train.shape[1] == 1:
                    y_train = y_train.ravel()
                clf.fit(X=X_train_normalized, y=y_train)
            # --------- test:
            # if regressor != 'NeuralNetwork':
            #     labels_predicted = clf.predict(X=X_test)
            # else:
            #     labels_predicted = clf.predict(X=X_test_normalized)
            labels_predicted = clf.predict(X=X_test_normalized)
            # recognition_rate = clf.score(X=X_test, y=y_test)
            recognition_rate = mean_absolute_error(y_true=y_test, y_pred=labels_predicted)
