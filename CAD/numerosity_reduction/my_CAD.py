...
class My_CAD:
    ...
    def sort_samples(self, scores, X, Y, kernel_approach):
        # X, Y --> rows are features and columns are samples
    ...
    def sort_samples_for_classification(self, X, Y, kernel_approach):
        # X --> rows are features and columns are samples
        n_dimensions = X.shape[0]
        ...
        for class_index in range(n_classes):
            ...
            if not kernel_approach:
                self.CAD_fit(X=samples_of_class, display_scree_plot=False)
                y_pred, scores = self.CAD_predict(X=samples_of_class)
            else:
                self.kernel_CAD_fit(X=samples_of_class, display_scree_plot=False)
                y_pred, scores = self.CAD_predict(X=samples_of_class)
            X_final_sorted_ofClass, Y_final_sorted_ofClass, scores_sorted_ofClass = self.sort_samples(scores=scores, X=samples_of_class, Y=labels_of_samples_of_class, kernel_approach=kernel_approach)
    ...
    def CAD_prototype_selection_for_classification(self, X, Y, kernel_approach):
        # X --> rows are features and columns are samples
        n_dimensions = X.shape[0]
        n_total_samples = X.shape[1]
        X_separated_classes, original_index_in_whole_dataset = self.separate_samples_of_classes_2(X=X, y=Y.ravel())
        n_classes = len(X_separated_classes)
        kept_prototypes_indices = np.zeros((1, n_total_samples))
        for class_index in range(n_classes):
            samples_of_class = X_separated_classes[class_index]
            samples_of_class2 = X_separated_classes[1]
            n_samples_of_class = samples_of_class.shape[1]
            labels_of_samples_of_class = np.ones((1, n_samples_of_class)) * class_index
            if not kernel_approach:
                self.CAD_fit(X=samples_of_class, display_scree_plot=False)
                ...
                self.kernel_CAD_fit(X=samples_of_class, display_scree_plot=False)
                y_pred, scores = self.CAD_predict(X=samples_of_class)
            kept_prototypes_indices_ofClass = self.CAD_prototype_selection(scores=scores, kernel_approach=kernel_approach)
            kept_prototypes_indices_ofClass = kept_prototypes_indices_ofClass.ravel()
            for sample_index_in_class in range(n_samples_of_class):
                original_index = original_index_in_whole_dataset[class_index][sample_index_in_class]
                if kept_prototypes_indices_ofClass[sample_index_in_class] == 0:
                    kept_prototypes_indices[:, original_index] = 0
                else:
                    kept_prototypes_indices[:, original_index] = 1
        return kept_prototypes_indices
